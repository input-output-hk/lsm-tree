...
newtype MergingRun h = SingleRun (Run h)
...
closeLevel hfs (Level mr rs) = do
  case mr of SingleRun r -> Run.removeReference hfs r
...
mkLevelsCache lvls = LevelsCache_ {
  ...
  where rs = V.concatMap (\(Level (SingleRun r) rs') -> V.cons r rs') lvls
...
addRunToLevels ::
  ...
  -> Levels (Handle h)
  -> m (Levels (Handle h))
addRunToLevels conf@TableConfig{..} hfs hbio root uniqC r0 reg levels =
  ...
  where
    ...
    go !ln rs (V.uncons -> Nothing) = do
        ...
        mr <- newMerge conf hfs hbio root uniqC policyForLevel Merge.LastLevel rs
    ...
    go !ln rs' (V.uncons -> Just (Level mr rs, ls)) = do
        ...
        case mergePolicyForLevel confMergePolicy ln ls of
          ...
          LevelTiering | runSize r <= maxRunSize' conf LevelTiering (ln - 1) -> do
            ...
            mr' <- newMerge conf hfs hbio root uniqC LevelTiering mergelast (rs' `V.snoc` r)
          ...
          LevelTiering | levelIsFull rs -> do
            mr' <- newMerge conf hfs hbio root uniqC LevelTiering Merge.MidLevel rs'
          ...
          LevelTiering -> do
            let mergelast = mergeLastForLevel ls
            mr' <- newMerge conf hfs hbio root uniqC LevelTiering mergelast rs'
          ...
          LevelLevelling -> do
            assert (V.null rs && V.null ls) $ pure ()
            mr' <- newMerge conf hfs hbio root uniqC LevelLevelling Merge.LastLevel (rs' `V.snoc` r)
    ...
    newMerge conf' hfs' hbio' root' uniqC' mergepolicy mergelast rs
      | Just (r, rest) <- V.uncons rs
      , V.null rest = do
          pure (SingleRun r)
      | otherwise = do
        assert (let l = V.length rs in l >= 2 && l <= 5) $ pure ()
        r <- allocateTemp reg (mergeRuns conf' hfs' hbio' root' uniqC' mergepolicy mergelast rs) (Run.removeReference hfs')
...
snapshot snap label th = do
    withOpenTable th $ \thEnv -> do
      ...
      -- consistent.
      let runNumbers = V.map (\(Level (SingleRun mr) rs) ->
                                ( runNumber (Run.runRunFsPaths mr)
      ...
      pure $! V.sum (V.map (\((_ :: Word64), rs) -> 1 + V.length rs) runNumbers)
...
open sesh label snap = do
    withOpenSession sesh $ \seshEnv -> do
      ...
      let runPaths = V.map (bimap (RunFsPaths (Paths.activeDir $ sessionRoot seshEnv))
                                  (V.map (RunFsPaths (Paths.activeDir $ sessionRoot seshEnv))))
                           runNumbers
      with (openLevels hfs runPaths) (newWithLevels seshEnv conf)

{-# SPECIALISE openLevels :: HasFS IO h -> V.Vector (RunFsPaths, V.Vector RunFsPaths) -> Managed IO (Levels (FS.Handle h)) #-}
